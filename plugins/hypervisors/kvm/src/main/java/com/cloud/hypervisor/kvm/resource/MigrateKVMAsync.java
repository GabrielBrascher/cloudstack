...
public class MigrateKVMAsync implements Callable<Domain> {
    ...
    private boolean isAnyVolumeOnManagedStorage;
    ...
    public MigrateKVMAsync(final LibvirtComputingResource libvirtComputingResource, final Domain dm, final Connect dconn, final String dxml,
                           final boolean migrateStorage, final boolean autoConvergence, final String vmName, final String destIp, boolean isAnyVolumeOnManagedStorage) {
    ...
    public Domain call() throws LibvirtException {
        ...
        if (migrateStorage && isAnyVolumeOnManagedStorage) {
            flags |= 1 << 6;
        }
