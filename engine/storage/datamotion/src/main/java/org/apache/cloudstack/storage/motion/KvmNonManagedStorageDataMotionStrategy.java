...
/**
 * Extends {@link StorageSystemDataMotionStrategy}, allowing KVM hosts to migrate VMs with the ROOT volume on a non managed file system storage pool.
...
public class KvmNonManagedStorageDataMotionStrategy extends StorageSystemDataMotionStrategy {
    ...
    /**
     * Uses the canHandle from the Super class {@link StorageSystemDataMotionStrategy}. If the storage pool is of FileSystem and the internalCanHandle from {@link StorageSystemDataMotionStrategy} CANT_HANDLE, returns the HIGHEST strategy priority. otherwise returns CANT_HANDLE.
     * Note that the super implementation (overrided) is called by {@link #canHandle(Map, Host, Host)} which ensures that {@link #internalCanHandle(Map)} will be executed only if the source host is KVM.
     */
    @Override
    ...
    /**
     * Returns a {@link MigrateDiskInfo} object configured for migrating a File System volume.
     */
    @Override
    ...
    /**
     * Calls {@link #generateLibvirtDestinyImagesPath(VirtualMachineTO, VolumeVO, Host, StoragePoolVO, VolumeInfo)} to generate the Libvirt local Filesystem path.
     */
    @Override
    protected String getDestPath(VirtualMachineTO vmTO, VolumeVO srcVolume, Host destHost, StoragePoolVO destStoragePool, VolumeInfo destVolumeInfo) {
        return generateLibvirtDestinyImagesPath(vmTO, srcVolume, destHost, destStoragePool, destVolumeInfo);
    }
